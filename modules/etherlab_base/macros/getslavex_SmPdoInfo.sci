...
function [smno,smindex,smexclude,smmandatory] = getslavex_SmPdoInfo(pdo_desc)
    //mprintf("getslavedesc_SmPdoInfo(%s, %d, %d) --> ", rootkey, direction, pdoindex);
    smno        = getnum(pdo_desc.attributes.Sm);
    smindex     = getnum(pdo_desc.attributes.Index);
    smexclude   = getnum(pdo_desc.attributes.Exlude);
    smmandatory = getnum(pdo_desc.attributes.Mandatory);
